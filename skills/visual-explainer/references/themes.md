...
Twelve prebuilt palettes and an optional runtime theme picker, for pages where the reader should be able to change the palette after the page is generated.
...
A theme here is one fixed palette, not a light/dark pair. Light and dark coverage comes from the theme list (7 dark, 5 light), and the picker is the reader's light/dark control. Do not wrap theme values in `@media (prefers-color-scheme: …)`; an explicit choice should not be overridden by the OS.
...
```js
const THEMES = {
  'entaina': { label: 'Entaina', mode: 'light', css: {
    '--bg':'#f1f6fa', '--surface':'#f5f5f5', '--surface-elevated':'#ffffff',
    '--border':'#d4d4d4', '--border-bright':'#bababa',
    '--text':'#353535', '--text-dim':'#717171',
    '--accent':'#6b859f', '--accent-dim':'rgba(107,133,159,.12)',
    '--node-a':'#6b859f', '--node-a-dim':'rgba(107,133,159,.12)',
    '--node-b':'#de865c', '--node-b-dim':'rgba(222,134,92,.14)',
    '--node-c':'#bd9947', '--node-c-dim':'rgba(248,207,115,.20)',
    '--green':'#008d3d', '--green-dim':'rgba(0,141,61,.10)',
    '--red':'#e7000b', '--red-dim':'rgba(251,44,54,.10)',
    '--orange':'#ca3500', '--orange-dim':'rgba(255,137,4,.14)'
  } },
...
```js
const FONT_PAIRS = {
  'entaina':    { label: 'Comfortaa + Lato',    sans: "'Lato'",                 mono: "'JetBrains Mono'", display: "'Comfortaa'" },
...
Swapping a palette means re-rendering every Mermaid diagram, because Mermaid bakes its colors into the SVG at render time. `../targets/page/exemplars/mermaid-flowchart.html` already exposes `window.rerenderDiagrams()` for this. Call it; do not write a second insertion path, and do not reach into the template's internals.
...
async function applyFont(id) {
  ...
  root.style.setProperty('--font-display', `${pair.display ?? pair.sans}, system-ui, sans-serif`);
...
These values seed `DEFAULT_THEME` and `DEFAULT_FONT` only. The reader can still change them, and an unrecognized or absent value falls back to what the page's aesthetic direction would have chosen anyway. This is an agent-readable generation contract, not a `scripts/render.mjs` parameter.
