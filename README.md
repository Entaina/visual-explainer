...
La skill es un directorio plano y portable (`skills/visual-explainer/`): markdown + assets + scripts Node sin dependencias, sin APIs de ningún arnés. Este repositorio la envuelve además como plugin de Claude Code y como paquete de pi.
...
### Codex (y otros arneses con AGENTS.md)
...
- Manifiestos de instalación: `.claude-plugin/plugin.json` (plugin de Claude Code) y `package.json` con la clave `pi` (paquete de pi).
